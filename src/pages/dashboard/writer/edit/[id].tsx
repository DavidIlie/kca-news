  ...
  return (
    <>
      ...
      {openWordCountOverlay && (
        ...
        >
          <span className="font-medium">{(readingTimeInfo as any).words}</span>{" "}
          Words
        </div>
      )}
      {/* {showContextMenu && (
          ...
          <div className="container mx-auto max-w-4xl">
            <div className="border-b-2 pb-4">
              {displayAlert && (
                ...
                >
                  {`${article.writer!.id === user.id ? "Your" : "This"} article
            ...
            <div className="items-center justify-evenly gap-3 sm:flex">
              ...
              <div className="sm:h-[300px] sm:w-1/2">
                {description !== article.description && (
                  <div className="relative hidden sm:block">
                    <RiRestartLine
                      className="absolute right-0 mt-5 -mr-10 cursor-pointer select-none text-2xl"
                      onClick={() => {
          ...
          <Consumer>
            {({ mobile }) => (
              <>
                ...
                {!mobile && (
                  <div className="borderColor flex items-center justify-between gap-2 border-b-2 px-4 py-4">
                    <div className={`${lastSaved && "-mb-2"}`}>
                      ...
                      {lastSaved && (
                        <Tooltip label="Save now" disabled={!canSave}>
                          <span
                            className={`text-xs font-normal text-gray-300 ${
                              canSave && "cursor-pointer hover:underline"
                            }`}
                            onClick={() =>
                              !canBeAttemptedSave && attemptToSave()
                            }
                          >
                            Saved{" "}
                ...
                >
                  {title === "" && (
                    <h1 className="mb-2 -mt-2 px-1 font-medium text-red-500">
                  ...
                  {user.isAdmin && (
                    <div className="mt-1 flex items-center gap-2">
                      <Radio
                        ...
                        disabled={
                          article.underReview || article.categoryId.length === 0
                        }
                        labelDisabled={
                          article.underReview || article.categoryId.length === 0
                        }
                ...
                >
                  ...
                  {location !== null && (
                    <MultiSelect
                      data={links
                        .filter((s) => s.location.includes(location))
                        .map((l) => ({
                          value: l.id,
                ...
                <EditorSettingsDisclosure name="Tags">
                  <MultiSelect
                    ...
                    searchable
                    onChange={(v) => {
                      if (v.length === 0 || v.length < tags.length) setTags(v);
                    }}
                    clearable
                    maxDropdownHeight={160}
                    maxSelectedValues={2}
                ...
                <EditorSettingsDisclosure name="Co Writers">
                  <MultiSelect
                    ...
                    onChange={(e) => {
                      if (e.length === 0) return setCoWriterSearchValue([]);
                      if (e.length > 1) return setCoWriterSearchValue([]);
                      ...
                      if (coWriters.filter((w) => w.id === user.id).length > 0)
                    ...
                    onSearchChange={async (v) => {
                      ...
                      const r = await fetch(
                        `/api/article/${
                          article.id
                        }/cowriter/search?query=${encodeURIComponent(v)}`,
                        {
                          method: "POST",
                          credentials: "include",
                        }
                      ...
                      if (r.status === 200) {
                        ...
                        users.forEach((user) => {
                          coWriters.forEach((writer) => {
                            if (user.id !== writer.id) final.push(user);
                          });
                        });
                    ...
                    >
                      <div className="flex select-none items-center gap-2">
                        <Image
                          ...
                          height="25px"
                          blurDataURL={shimmer(1920, 1080)}
                          alt={`${writer.name.split(" ")[0]}'s profile image`}
                        />
                      ...
                      <BsTrash
                        ...
                        onClick={() => {
                          setCoWriterSearch([]);
                          setCoWriters(
                            coWriters.filter((user) => user.id !== writer.id)
                          );
                        }}
                      />
                    </div>
                ...
                {/* <EditorSettingsDisclosure name="Transfer">
                        </EditorSettingsDisclosure> */}
                {!article.published && (
                        ...
                        <Button
                          ...
                          onClick={() =>
                            clipboard.copy(
                              ...
                              }${createArticleUrl(article)}?share=${
                                article.sharedId
                              }`
                            )
                          }
                ...
                {(article.upvotes!.length > 0 || article.downvotes!.length) >
                  0 && (
                  <EditorSettingsDisclosure name="Opinion">
                    <>
                      <h1>
                        {article.writer!.id !== user.id ? "This" : "Your"}{" "}
                        article has {article.upvotes!.length} like
                        {article.upvotes!.length !== 1 && "s"} and{" "}
                        {article.downvotes!.length} dislike
                        {article.downvotes!.length !== 1 && "s"}.
                      </h1>

                      <Tabs className="-px-1 mt-1" grow position="center">
                        <Tabs.Tab label="Likes" icon={<AiOutlineLike />}>
                                ...
                                (
                                  ...
                                  <div
                                    className={`flex items-center gap-2 ${
                                      index !== article.upvotes!.length - 1 &&
                                      "mb-2"
                                    }`}
                                    key={index}
                                  >
                                ...
                                (
                                  ...
                                  >
                                    <Image
                                      ...
                                      placeholder="blur"
                                      blurDataURL={shimmer(100, 100)}
                                      src={downvote.user!.image}
                                      className="w-[20%] rounded-full"
                                    />
                                    <Link
                                      href={`/profile/${downvote.user!.id}`}
                                    >
                                      <a className="font-medium duration-150 hover:text-blue-500">
                                        {computeKCAName(downvote.user!)}
                                      </a>
                                    </Link>
                                  </div>
                                )
      ...
      <ConfirmModal
        ...
        updateModalState={() => {
          if (openConfirmModalUnderReview)
            return setOpenConfirmModalUnderReview(!openConfirmModalUnderReview);
...
const CoWriterUser = forwardRef<HTMLDivElement, any>(
  ({ user, ...rest }: { user: User }, ref) => (
    <div
      ref={ref}
      {...rest}
      className="flex cursor-pointer select-none items-center gap-2 px-2 py-2 duration-150 hover:bg-gray-300 dark:hover:bg-dark-bg"
    >
      <Image
        className="rounded-full"
        src={user.image}
      ...
      : await prisma.article.findFirst({
          where: {
            ...
            OR: [
              ...
              { coWriters: { some: { id: session?.user?.id } } },
            ],
          },
          include: {
            writer: true,
